...
namespace QSDMS.Business
{
    ...
    /// 描 述：用户组管理
    /// </summary>
    public class UserGroupBLL:BaseBLL<IUserGroupService>
    {
      
        /// <summary>
        /// 访问实例
        /// </summary>
        public static UserGroupBLL m_Instance = new UserGroupBLL();
        ...
        public void SaveForm(string keyValue, RoleEntity userGroupEntity)
        {
            try
            {
                InstanceDAL.SaveForm(keyValue, userGroupEntity);
                CacheFactory.Cache().RemoveCache(cacheKey);
            }
            catch (Exception)
            {
