...
.directive("lrappComponents", ["$rootScope", "$lrmComponents", function ($rootScope, $lrmComponents) {
    return {
        restrict: "E",
        replace: true,
        template: '<div ng-repeat="component in $root.componentGroups" class="btn-item-row btn-component" data-value="{{component.id}}"><i class="fa {{component.icon}}"></i>{{component.text}}</div>',
...
.directive('lrappBox', function () {
    return {
        ...
        transclude: true,
        template:
        '<div class="btn-box expanded">' +
        '<h4 class="btn-box-toggle"></h4>' +
        '<div class="btn-box-content" ng-transclude></div>' +
        '</div>',
        link: function (scope, element, attr) {
            var toggle = element.find('.btn-box-toggle');
    ...
    return {
        init: function (projectId) {
            ...
            if (designer.projectId == "") {
                ...
                .success(function (data) {
                    designer.projectName = data.Name;
                    designer.projectIcon = data.Icon;
                })
                .error(function () {
                    designer.projectId = "";
...
(function ($) {
    function lrAppBox(name, strhtml) {
        var html = '<div class="btn-box expanded">';
        html += '<h4 class="btn-box-toggle" onClick="lrappbox(this)"><span class="caret"></span>' + name + '</h4>';
        html += '<div class="btn-box-content">' + strhtml + '</div>';
        html += '</div>';
        return html;
    }
    ...
    var _tempsAttr = {//模板属性设置
        lrTabs: function (data) {
            ...
            _html += '<div class="btn-app-right-header"><span class="btn-app-title">' + data.text + '</span></div>';
        ...
        lrTab: function (data,designer) {
            ...
            _html += '<div class="btn-app-right-header"><span class="btn-app-typePage"><a>Tabs控制器</a><span class="btn-app-slash">/</span></span>' + data.text + '</div>';
        ...
        lrPageBlank: function (data) {
            ...
            _html += '<div class="btn-box" style="padding-top:10px;">';
        ...
        lrHeader: function (data, designer) {
            ...
            _html += '<div class="btn-app-right-header"><span class="btn-app-typePage"><a>' + designer.currentPage.text + '</a><span class="btn-app-slash">/</span></span>' + data.text + '</div>';
    ...
    $.webappDesigner = {
        attributes: function (data) {
            ...
            $('.btn-app-right-bar').html(strhtml).ready(function () {
                $('.color').colorpicker();
                ...
                $('.btn-app-right-bar input').unbind();
                $('.btn-app-right-bar input').bind('input propertychange', function () {
                    var $obj = $(this);
                    var _value = $obj.val();
                    var _name = $obj.attr("data-name");
                    if (_name == "routing")
                ...
                $('.btn-app-right-bar textarea').unbind();
                $('.btn-app-right-bar textarea').bind('valuechange', function () {
                    var $obj = $(this);
                    var _value = $obj.val();
                    var _name = $obj.attr("data-name");
                    data.currentFocus.setValue(_name, _value, data.currentFocus);
