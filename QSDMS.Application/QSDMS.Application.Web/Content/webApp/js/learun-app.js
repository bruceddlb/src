...
.controller('modelCtrl', ['$rootScope', '$scope', '$state', '$ionicScrollDelegate', '$lrComponent', '$lrDoBroadcastToparent', function ($rootScope, $scope, $state, $ionicScrollDelegate, $lrComponent, $lrDoBroadcastToparent) {
    ...
    if ($rootScope.iframeLoaded != true) {
        $rootScope.$on('init', function (event, data) {
            ...
            //初始化设计器
            parent.$('.btn-app-componentContent .btn-component').draggable({
            ...
            $('ion-content .scroll').sortable({
                opacity: 0.6,
                cursor: 'move',
                placeholder: "btn-component-state",
                start: function (event, ui) {
                    var $this_place = $(ui.item[0]);
                    var componentType = $this_place.attr('data-value');
                    $(".btn-component-state").addClass('btn-component-item');
                    $(".btn-component-state").html(sortableStartComponent(componentType, $this_place.children()));
                ...
                stop: function (event, ui) {
                    ...
                    $this_place.addClass('btn-component-item');
                    var componentType = $this_place.attr('data-value');
            ...
            switch (data.cmd) {
                ...
                case "removePage":
                    ...
                    for (var i in data.object.ChildNodes)
                    {
                        ...
                        var $node = $('<div class="btn-component-item" data-value="' + _node.attr.value + '" ></div>');
...
function sortableStartComponent(componentType,node) {
    ...
    switch (componentType) {
        case "lrHeader":
            if (node.hasClass("fa")) {
                componentHtml = '<h1 class="btn-component-heading">标题</h1>';
            }
            else {
                componentHtml = '<h1 class="btn-component-heading">' + node.text() + '</h1>';
            }
            
            break;
        case "lrParagraph":
            if (node.hasClass("fa")) {
                componentHtml = '<p class="btn-component-one">请填写内容</p>';
            }
            else {
                componentHtml = '<p class="btn-component-one">' + node.html() + '</p>';
            }
            break;
        case "lrBtn":
            if (node.hasClass("fa")) {
        ...
        case "lrList3":
            if (node.hasClass("fa")) {
                componentHtml = '<div class="btn-list-type3"><div class="item item-icon-left" style="border:1px dashed #ccc;color:#ccc">';
        ...
        case "lrList4":
            if (node.hasClass("fa")) {
                componentHtml = ' <ion-item class="item-icon-right btn-iconitem item" style="border:1px dashed #ccc;">';
                ...
                componentHtml = ' <ion-item class="item-icon-right btn-iconitem item"  style="border:1px dashed #ccc;">' + node.html() + '</ion-item>';
            }
            break;
    }
...
function sortableStopComponent(componentType) {
    ...
    switch (componentType) {
        case "lrHeader":
            componentHtml = '<btn-component-header></btn-component-header>';
            break;
    }
    return componentHtml;
}
