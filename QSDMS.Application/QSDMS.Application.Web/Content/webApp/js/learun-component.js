...
.factory('$lrComponent', ['$rootScope', '$lrfnGuid', '$lrDoBroadcastToparent', function ($rootScope, $lrfnGuid, $lrDoBroadcastToparent) {
    return{
        lrHeader: function ($div, sortable, componentData) {
            var isNewed = $div.find('.btn-component-heading').length;
            if (isNewed == 0) {
                ...
                $div.html('<h1  class="btn-component-heading btn-f ">' + component.text + '</h1>');
                $obj = $div.find('.btn-component-heading');
                ...
                $div.find('.btn-component-heading').trigger('click');
            }
        },
        lrParagraph: function ($div, sortable, componentData) {
            var isNewed = $div.find('.btn-component-one').length;
            if (isNewed == 0) {
                var _setAttrLr = function (data) {
                    switch (data.attrName) {
                        case "content":
                ...
                $div.html('<p class="btn-component-one btn-f ">' + component.content + '</p>');
                $obj = $div.find('.btn-component-one');
                $obj.attr('data-value', component.id);
                var $selectBox = $('.ic-select-box');
                $rootScope.$on('componentScroll', function (event, data) {
                    if ($selectBoxCurrent != null) {
                ...
                $div.find('.btn-component-one').trigger('click');
            }
        },
        ...
        lrList3: function ($div, sortable, componentData) {
            ...
            var isNewed = $div.find('.btn-list-type3').length;
            if (isNewed == 0) {
                var _setAttrLr = function (data) {
                    switch (data.attrName) {
                        case "name":
                ...
                var _chtml = '<div class="btn-list-type3"><div class="item item-icon-left" >';
                ...
                $div.find('.btn-list-type3').trigger('click');
            }
        },
        lrList4: function ($div, sortable, componentData) {
            var isNewed = $div.find('.btn-iconitem').length;
                if (isNewed == 0) {
                    var _setAttrLr = function (data) {
                        switch (data.attrName) {
                            case "name":
                    ...
                    var _chtml = ' <ion-item class="item-icon-right btn-iconitem item" >';
                    ...
                    $div.find('.btn-iconitem').trigger('click');
                }
        }
    };
}])
