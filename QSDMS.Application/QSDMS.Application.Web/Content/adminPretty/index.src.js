var tablist = {
    newTab: function (item) {
        ...
        $('.menuTab').each(function () {
            if ($(this).data('id') == dataUrl) {
                if (!$(this).hasClass('active')) {
                    ...
                    $.dmstab.scrollToTab(this);
                    $('#mainContent .DMS_iframe').each(function () {
                        if ($(this).data('id') == dataUrl) {
                            $(this).show().siblings('.DMS_iframe').hide();
                            return false;
                        }
                    });
                }
        ...
        if (flag) {
            ...
            var str1 = '<iframe class="DMS_iframe" id="tabs_iframe_' + dataId + '" name="tabs_iframe_' + dataId + '"  width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" seamless></iframe>';
            $('#mainContent').find('iframe.DMS_iframe').hide();
            ...
            $.dmstab.scrollToTab($('.menuTab.active'));
        }
...
(function ($) {
    ...
    $.dmstab = {
        ...
        refreshTab: function () {
            ...
            var target = $('.DMS_iframe[data-id="' + currentId + '"]');
        ...
        closeOtherTabs: function () {
            $('.page-tabs-content').children("[data-id]").find('.tab_close').parents('a').not(".active").each(function () {
                $('.DMS_iframe[data-id="' + $(this).data('id') + '"]').remove();
                $(this).remove();
            });
        ...
        addTab: function () {
            ...
            if (flag) {
                ...
                $('.menuTab').removeClass('active');
                var str1 = '<iframe class="DMS_iframe" id="tabs_iframe_' + dataId + '" name="tabs_iframe_' + dataId + '"  width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" style="display: inline;"></iframe>';
                $('#mainContent').find('iframe.DMS_iframe').hide();
                $('#mainContent').append(str1);
                Loading(true);
                $('#mainContent iframe:visible').load(function () {
        ...
        scrollTabRight: function () {
            ...
            var tabOuterWidth = $.dmstab.calSumWidth($(".dms-tabs").children().not(".menuTabs"));
            var visibleWidth = $(".dms-tabs").outerWidth(true) - tabOuterWidth;
            var scrollVal = 0;
            if ($(".page-tabs-content").width() < visibleWidth) {
                ...
                scrollVal = $.dmstab.calSumWidth($(tabElement).prevAll());
                if (scrollVal > 0) {
                    $('.page-tabs-content').animate({
        ...
        scrollTabLeft: function () {
            ...
            if ($(".page-tabs-content").width() < visibleWidth) {
                ...
                offsetVal = 0;
                if ($.dmstab.calSumWidth($(tabElement).prevAll()) > visibleWidth) {
                    while ((offsetVal + $(tabElement).outerWidth(true)) < (visibleWidth) && tabElement.length > 0) {
                        offsetVal += $(tabElement).outerWidth(true);
        ...
        scrollToTab: function (element) {
            var marginLeftVal = $.dmstab.calSumWidth($(element).prevAll()), marginRightVal = $.dmstab.calSumWidth($(element).nextAll());
            var tabOuterWidth = $.dmstab.calSumWidth($(".dms-tabs").children().not(".menuTabs"));
            var visibleWidth = $(".dms-tabs").outerWidth(true) - tabOuterWidth;
            var scrollVal = 0;
        ...
        init: function () {
            $('.menuTabs').on('click', '.menuTab .tab_close', $.dmstab.closeTab);
            $('.menuTabs').on('click', '.menuTab', $.dmstab.activeTab);

            $('.tabLeft').on('click', $.dmstab.scrollTabLeft);
            $('.tabRight').on('click', $.dmstab.scrollTabRight);
            $('.tabReload').on('click', $.dmstab.refreshTab);
            ...
            $('.tabCloseAll').on('click', function () {
                ...
                $('.page-tabs-content').children("[data-id]:first").each(function () {
                    $('.DMS_iframe[data-id="' + $(this).data('id') + '"]').show();
                    $(this).addClass("active");
                });
                $('.page-tabs-content').css("margin-left", "0");
            });
            $('.tabCloseOther').on('click', $.dmstab.closeOtherTabs);
            $('.fullscreen').on('click', function () {
                if (!$(this).attr('fullscreen')) {
                    $(this).attr('fullscreen', 'true');
                    $.dmstab.requestFullScreen();
                } else {
                    $(this).removeAttr('fullscreen');
                    $.dmstab.exitFullscreen();
                }
            });
    ...
    $.dmsindex = {
        ...
        loadMenu: function (isInit) {
            var flag = false;
            var topMenuWidth = $('.dms-Head').width() - $.dmstab.calSumWidth($(".dms-Head").children().not(".left-bar"));
            ...
            $.each(data, function (i) {
                ...
                if (row.ParentId == "0") {
                    ...
                    else if ((menuWidth + 92) > topMenuWidth) {
                        _html1 = _itemHtml;
                    }
                    else {
                        _html += _itemHtml;
            ...
            if (isInit || flag) {
                ...
                $('.menuiframe').unbind();
                $('.menuiframe').on('click', $.dmstab.addTab);
